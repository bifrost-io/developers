---
title: vMOVR
description: Understanding vMOVR
---

## What is vMOVR?

vMOVR (voucher GLMR) is a shadow token of staked GLMR, with fully underlying GLMR reserve and yield-bearing feature of GLMR staking reward. Users can deposit GLMR into Bifrost SLP protocol and get vMOVR as return, vMOVR can be traded in the open market or be redeemed back to GLMR. Holding vMOVR equals to holding the GLMR staking position, delegating rewards appreciate the exchange price of vMOVR.

<Callout title="Staking rewards">
Delegating rewards automatically add to the vMOVR exchange price, no manual claim. The longer vMOVR postion holding, the greater amount of GLMR will be exchanged back for redemption.
</Callout>
...
<Callout type="success">
**Floating redemption period, vMOVR ≤ 168 hours, vMOVR ≤ 48 hours**
</Callout>
...
vMOVR is minted by Bifrost SLP pallet, so firstly users have to call XCM cross chain transfer GLMR from Moonriver to Bifrost Parachain.

### Mint vMOVR

1. Users initiate a vMOVR mint order, SLP protocol transfers GLMR to GLMR Ready Pool (which is an order pool accumulates all mint and redeem orders), SLP mints vMOVR for users;&#x20;
...
The GLMR reward will be reinvested on Moonriver, and the Oracle will transmit the reward data to the GLMR Ready Pool to adjust the vMOVR exchange rate.

vMOVR rate=(StakedGLMR+StakingReward)/vMOVR Allocation
